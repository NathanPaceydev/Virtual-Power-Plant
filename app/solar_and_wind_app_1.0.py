...
def get_input_data(plot_canvas):
    ...
    if response.status_code == 200:
        ...
        # get Station Info and Display
        station_info = data['station_info']
        latitude = station_info["lat"]
        longitude = station_info["lon"]
        
        lat_lon_frame = ttk.Frame(plot_canvas_frame)  # Add background color for visibility
        lat_lon_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=10)

        latitude_label = ttk.Label(lat_lon_frame, text=f"Latitude: {latitude}", background='lightblue')
        latitude_label.pack(side=tk.LEFT, padx=5, pady=5)

        longitude_label = ttk.Label(lat_lon_frame, text=f"Longitude: {longitude}", background='lightgreen')
        longitude_label.pack(side=tk.LEFT, padx=5, pady=5)
